...
Args:
    ...
    W1: 1D array of first layer weights, of shape
        (input_dim x hidden_dim)  n*l
    W2: 1D array of second layer weights, of shape
        (hidden_dim x num_classes) l*k
    m: num_examples
    n: input_dim
    l: hidden_dim
    k: num_classes
...
void nn_epoch_cpp(const float *X, const unsigned char *y, float *W1, float *W2, size_t m, size_t n, size_t l, size_t k, float lr, size_t batch)
{
    ...
    for (size_t i = 0; i < m; i += batch)
    {
        ...
        matrix_dot(X_b, W1, Z1, batch, n, l);
        ...
        matrix_mul(G1, mask, batch*l);
        matrix_dot_trans(X_b, G1, W1_l, batch, n, l);
        matrix_dot_trans(Z1, Z2, W2_l, batch, l, k);
...
void train_nn(const DataSet *train_data, const DataSet *test_data, size_t num_classes, size_t hidden_dim, size_t epochs, float lr, size_t batch)
{
    ...
    for (size_t epoch = 0; epoch < epochs; epoch++)
    {
        ...
        matrix_dot(train_data->images_matrix, W1, temp1, train_data->images_num, train_data->input_dim, hidden_dim);
        matrix_dot(test_data->images_matrix, W1, temp2, test_data->images_num, test_data->input_dim, hidden_dim);
        for(size_t i=0; i<train_data->images_num * hidden_dim; i++){
            if(temp1[i]<0){
                temp1[i] = 0;
            }
        }
        matrix_dot(temp1, W2, train_result, train_data->images_num, hidden_dim, num_classes);
        ...
        matrix_dot(temp2, W2, test_result, test_data->images_num, hidden_dim, num_classes);
